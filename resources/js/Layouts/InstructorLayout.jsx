            ...
            <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f8fbfa]/95 px-4 py-4 backdrop-blur lg:px-8">
                <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-slate-500">Panel Dosen</p>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex size-11 items-center justify-center rounded-full bg-emerald-700 text-sm font-semibold text-white">
                            {auth?.user?.name?.slice(0, 2).toUpperCase() ?? 'DS'}
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                ...
                <nav aria-label="Navigasi dosen" className="mx-auto mt-4 flex max-w-7xl gap-2 overflow-x-auto">
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`inline-flex h-10 shrink-0 items-center gap-2 rounded-lg px-3 text-sm font-medium transition focus:outline-none focus:ring-3 focus:ring-emerald-600/20 ${
